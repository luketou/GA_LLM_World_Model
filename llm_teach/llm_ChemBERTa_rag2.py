...
def run(args):
    ...
    def calibrate_history(upto_gen: Optional[int]):
        if upto_gen is None or upto_gen < 0:
            return None
        subset = {k: v for k, v in gens.items() if k <= upto_gen and len(v) > 0}
        if not subset:
            return None
        end = max(subset.keys())
        if args.calibrate_window and args.calibrate_window > 0:
            start = max(min(subset.keys()), end - args.calibrate_window + 1)
            subset = {k: v for k, v in subset.items() if start <= k <= end}
        else:
            start = min(subset.keys())
        try:
            a, b = calibrate_alpha_beta(
                subset, enc, start_gen=start, end_gen=end, folds=args.folds,
    ...
    last_calibrated_upto: Optional[int] = None
    ...
    for g in sorted(gens.keys()):
        if args.calibrate and args.calibrate_interval > 0:
            ...
            if len(history_g) >= args.calibrate_min_history and (g % args.calibrate_interval == 0) and (last_calibrated_upto != g - 1):
                res = calibrate_history(g - 1)
                if res:
                    alpha, beta, hist_start, hist_end = res
                    last_calibrated_upto = hist_end
                    last_calibrated_meta = (hist_start, hist_end)
...
def build_argparser():
    ...
    ap.add_argument("--calibrate-window", type=int, default=0,
                    help="Limit calibration to the most recent N generations (0=all history).")
    ap.add_argument("--calibrate-min-history", type=int, default=1,
                    help="Minimum historical generations required before (re)calibration.")
